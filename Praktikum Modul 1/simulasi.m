...
Gain = 0.8123;
Tau = 0.0849;
DeadTime = 0.0291; % can't be zero

% Waktu
K = 10; % Faktor pengali untuk memperpanjang atau memperpendek durasi simulasi
if Tau>DeadTime
    FinalTime = K*Tau;
...
title(['Kecepatan pada SP = ' num2str(SetPoint) ', Kp = ' num2str(Kp) ...
        ', Ki = ' num2str(Ki) ', Kd = ' num2str(Kd) ', dt = ' num2str(SimulationSampling)])

figure()
stairs(t(1:end-dlay-1),ManipulatedVariable,'b');
title(['MV pada SP = ' num2str(SetPoint) ', Kp = ' num2str(Kp) ...
        ', Ki = ' num2str(Ki) ', Kd = ' num2str(Kd) ', dt = ' num2str(SimulationSampling)])

figure()
stairs(t(1:end-dlay-1),AccumulationError,'b');
title(['Akumulasi error pada SP = ' num2str(SetPoint) ', Kp = ' num2str(Kp) ...
        ', Ki = ' num2str(Ki) ', Kd = ' num2str(Kd) ', dt = ' num2str(SimulationSampling)])
